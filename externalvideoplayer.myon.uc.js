...
// @author         ywzhaiqi
// @modify         Myon<myon.cn@gmail.com>
// @mod content    在b站网址弹出播放器后自动加载弹幕
...
(function(){
    ...
    var ns = window.externalVideoPlayer = {
        ...
        launchPlayer: function(listFileOrUrl){
            ...
            try{
            if(content.location.href.match(/www\.bilibili\.tv\/video\/av\d+/)){
                var argsSplit = args[0].split("\\");
                var cfilename = argsSplit[argsSplit.length-1]
                    .replace(/\..*$/,".ass");
                argsSplit.pop();
                var cpath = argsSplit.join("\\");
                // alert(cpath);
                bilibili.convert(cpath,cfilename,function(file){
                    args.push("/sub");
                    args.push(file);
                    // alert(args.join(" "));
